...
export class AnalyticsService {
    ...
    static async getSalesAnalytics(tenantId) {
        try {
            const allTransactionsRaw = await db
                .select({
                id: transactions.id,
    ...
    static async getProductAnalytics(tenantId) {
        try {
            const allTxItems = await db
                .select({
                productId: products.id,
    ...
    static async getProfitLoss(tenantId) {
        try {
            const allTxItems = await db
                .select({
                createdAt: transactions.createdAt,
            ...
            for (const item of allTxItems) {
                ...
                if (item.createdAt) {
                    ...
                    if (txDateObj >= thirtyDaysAgo) {
                        const dateStr = txDateObj.toISOString().split('T')[0];
                        if (!chartMap[dateStr])
                            chartMap[dateStr] = { revenue: 0, cogs: 0 };
    ...
    static async getCustomerAnalytics(tenantId) {
        try {
