...
import { 
  products, stockMovements, productStocks, outlets,
  stockAdjustments, stockAdjustmentItems,
  stockOpnameSessions, stockOpnameItems,
...
export class InventoryService {
  ...
  async createOutlet(tenantId: string, name: string, address: string) {
  ...
  async getProductStockByOutlet(tenantId: string, outletId: string) {
  ...
  async recordStockMovement(tenantId: string, outletId: string, userId: string, payload: {
    productId: string,
    type: 'in' | 'out',
    quantity: number,
    reason: string
    ...
    return await db.transaction(async (tx) => {
      ...
      let ps = await tx.select().from(productStocks).where(
        and(eq(productStocks.productId, payload.productId), eq(productStocks.outletId, outletId))
      ).limit(1);
  ...
  // 3. Stock Adjustments
  async getAdjustments(tenantId: string, outletId: string) {
  ...
  async createAdjustment(tenantId: string, outletId: string, userId: string, items: Array<{productId: string, adjustedStock: number, reason: string}>) {
    return await db.transaction(async (tx) => {
      ...
      for (const item of items) {
        let ps = await tx.select().from(productStocks).where(
          and(eq(productStocks.productId, item.productId), eq(productStocks.outletId, outletId))
        ).limit(1);
  ...
  async approveAdjustment(tenantId: string, adjustmentId: string, userId: string) {
    return await db.transaction(async (tx) => {
      const adj = await tx.select().from(stockAdjustments).where(eq(stockAdjustments.id, adjustmentId)).limit(1);
      if (adj.length === 0 || adj[0].status !== 'pending') throw new AppError('Invalid adjustment', 400);
  ...
  // 4. Stock Opname
  async createOpname(tenantId: string, outletId: string, userId: string) {
  ...
  async completeOpname(tenantId: string, opnameId: string, userId: string, items: Array<{productId: string, actualQuantity: number}>) {
    return await db.transaction(async (tx) => {
      const opn = await tx.select().from(stockOpnameSessions).where(eq(stockOpnameSessions.id, opnameId)).limit(1);
      if (opn.length === 0 || opn[0].status !== 'in_progress') throw new AppError('Invalid opname session', 400);
      ...
      for (const item of items) {
        let ps = await tx.select().from(productStocks).where(
          and(eq(productStocks.productId, item.productId), eq(productStocks.outletId, opn[0].outletId))
        ).limit(1);
  ...
  // 5. Stock Transfers
  async createTransfer(tenantId: string, sourceOutletId: string, destinationOutletId: string, userId: string, items: Array<{productId: string, quantity: number}>) {
    return await db.transaction(async (tx) => {
      ...
      for (const item of items) {
        ...
        let ps = await tx.select().from(productStocks).where(
          and(eq(productStocks.productId, item.productId), eq(productStocks.outletId, sourceOutletId))
        ).limit(1);
        
        const currentStock = ps.length > 0 ? parseInt(ps[0].stockQuantity || '0') : 0;
        if (currentStock < item.quantity) throw new AppError(`Insufficient stock in source outlet for product ${item.productId}`, 400);
  ...
  async receiveTransfer(tenantId: string, transferId: string, userId: string) {
    return await db.transaction(async (tx) => {
      ...
      if (trn.length === 0 || trn[0].status !== 'pending') throw new AppError('Invalid transfer', 400);
      ...
      for (const item of items) {
        // Add to destination
        let ps = await tx.select().from(productStocks).where(
          and(eq(productStocks.productId, item.productId), eq(productStocks.outletId, trn[0].destinationOutletId))
        ).limit(1);
