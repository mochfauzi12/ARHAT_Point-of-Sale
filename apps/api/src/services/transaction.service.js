...
import { transactions, transactionItems, payments, stockMovements, products, customers, shifts, boms, rawMaterialStocks } from '../models';
export class TransactionService {
    /**
     * List recent transactions
     */
    static async listTransactions(tenantId) {
    ...
    static async createTransaction(tenantId, cashierId, payload) {
        ...
        return await db.transaction(async (tx) => {
            // Find active shift
            const activeShift = await tx.select().from(shifts).where(and(eq(shifts.tenantId, tenantId), eq(shifts.cashierId, cashierId), eq(shifts.status, 'open'))).limit(1);
    ...
    static async checkout(tenantId, transactionId, paymentMethod, amount) {
        return await db.transaction(async (tx) => {
            ...
            const updatedTx = await tx.update(transactions)
                ...
                .where(and(eq(transactions.id, transactionId), eq(transactions.tenantId, tenantId))).returning();
            if (!updatedTx.length)
     ...
     * Hold a transaction
     */
    static async holdTransaction(tenantId, cashierId, payload) {
     ...
     */
    static async getHeldTransactions(tenantId) {
        const held = await db.select().from(transactions).where(and(eq(transactions.tenantId, tenantId), eq(transactions.status, 'held')));
     ...
     */
    static async resumeTransaction(tenantId, transactionId) {
        const updatedTx = await db.update(transactions)
            .set({ status: 'pending' })
            .where(and(eq(transactions.id, transactionId), eq(transactions.tenantId, tenantId), eq(transactions.status, 'held'))).returning();
        if (!updatedTx.length)
     ...
     */
    static async refundTransaction(tenantId, transactionId) {
        return await db.transaction(async (tx) => {
            // 1. Update status
            const updatedTx = await tx.update(transactions)
                .set({ status: 'refunded' })
                .where(and(eq(transactions.id, transactionId), eq(transactions.tenantId, tenantId), eq(transactions.status, 'completed'))).returning();
            if (!updatedTx.length)
     ...
     */
    static async voidTransaction(tenantId, transactionId) {
        const updatedTx = await db.update(transactions)
            .set({ status: 'voided' })
            .where(and(eq(transactions.id, transactionId), eq(transactions.tenantId, tenantId))).returning();
        if (!updatedTx.length)
