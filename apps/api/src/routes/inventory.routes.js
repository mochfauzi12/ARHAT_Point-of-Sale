...
inventoryRoutes.post('/outlets', async (c) => {
    ...
    if (!body.name)
        throw new AppError('Outlet name is required', 400);
...
inventoryRoutes.post('/movements', async (c) => {
    ...
    const result = await inventoryService.recordStockMovement(user.tenantId, body.outletId, user.id, { productId: body.productId, type: body.type, quantity: Number(body.quantity), reason: body.reason || '' });
    return c.json({ success: true, data: result });
});
// 4. Adjustments
