...
router.get('/', async (c) => {
    ...
    if (search) {
        query = query.where(or(ilike(customers.name, `%${search}%`), ilike(customers.phone, `%${search}%`)));
    }
...
router.get('/:id', async (c) => {
    ...
    const tenantId = c.get('tenantId');
    const result = await db.select().from(customers).where(or(eq(customers.id, id), eq(customers.tenantId, tenantId)) // wait, needs AND
    );
    // let's do properly
...
router.get('/:id/transactions', async (c) => {
    ...
    const txList = await db.select()
        .from(transactions)
        .where(and(eq(transactions.tenantId, tenantId), eq(transactions.customerId, id), eq(transactions.status, 'completed')))
